...
class WriteStack1(entries: Int = 8, AXItoTLEdgeIn: AXI4EdgeParameters, AXItoTLEdgeOut: TLEdgeOut, AXItoTLBundleIn: AXI4Bundle, AXItoTLBundleOut: TLBundle)(implicit p: Parameters) extends AXItoTLModule {
  val io = IO(new Bundle() {
    val in = new Bundle() {
      val aw = Flipped(DecoupledIO(new AXI4BundleAW(AXItoTLEdgeIn.bundle)))
      val w = Flipped(DecoupledIO(new AXI4BundleW(AXItoTLEdgeIn.bundle)))
      val b = DecoupledIO(new AXI4BundleB(AXItoTLEdgeIn.bundle))
    }
    val out = new Bundle() {
      val a = DecoupledIO(new TLBundleA(AXItoTLEdgeOut.bundle))
      val d = Flipped(DecoupledIO(new TLBundleD(AXItoTLEdgeOut.bundle)))
    }
  })
  ...
  val writeDataStack = Module(new SRAMTemplate(gen = new WSBlock, set = entries, way = 1, singlePort = true))
  ...
  */ val canW = Cat(writeStack.map(e => e.wvalid && e.wstatus === waitW && e.waitWFifoId === 0.U)).orR
  ...
  writeStack.foreach { case e => when(e.wvalid && e.wstatus === waitW && e.waitWFifoId === 0.U) {
    wsbIdx := e.entryid
  }
  }
  ...
  wreqArb.io.in zip writeStack foreach { case (in, e) => in.valid := e.wvalid && e.wstatus === sending && e.entryFifoid === 0.U && e.wready
    in.bits := e
  }
  ...
  /* ======== Receive d resp and Send b resp ======== */ val canRecD = Cat(writeStack.map(e => e.wvalid && e.wready && e.wstatus === waitDResp)).orR
  ...
  sendBArb.io.in zip writeStack foreach { case (in, e) => in.valid := e.wvalid && e.wstatus === sendB && e.waitSendBRespFifoId === 0.U
    in.bits := e
  }
  ...
  when(io.in.b.fire) {
    writeStack.foreach { case e => when(e.wvalid && e.wstatus === sendB && e.waitSendBRespFifoId === 0.U) {
      e.wvalid := false.B
      e.wstatus := idel
      e.wready := false.B
    }
...
/* ======== diplomacy ======== */ class WriteStack(entries: Int = 8)(implicit p: Parameters) extends AXItoTLModule {
  val io = IO(new Bundle() {
    val in = new Bundle() {
      val aw = Flipped(DecoupledIO(new AXI4BundleAW(edgeIn.bundle)))
      val w = Flipped(DecoupledIO(new AXI4BundleW(edgeIn.bundle)))
      val b = DecoupledIO(new AXI4BundleB(edgeIn.bundle))
    }
    val out = new Bundle() {
  ...
  */ val canW = Cat(writeStack.map(e => e.wvalid && e.wstatus === waitW && e.waitWFifoId === 0.U)).orR
  ...
  private val sourceD = io.out.d.bits.source
  private val wsIdx = sourceD(axi2tlParams.wbufIdBits - 1, 0)
  when(d_valid) {
    writeStack(wsIdx).wstatus := sendB
    writeStack(wsIdx).d_resp := Mux(io.out.d.bits.denied || io.out.d.bits.corrupt, AXI4Parameters.RESP_SLVERR, AXI4Parameters.RESP_OKAY)
  }
