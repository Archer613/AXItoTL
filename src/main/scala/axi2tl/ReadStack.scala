...
class ReadStack(entries: Int = 8)(implicit p: Parameters) extends AXItoTLModule {
  val io = IO(new Bundle() {
    val in = new Bundle() {
      val ar = Flipped(DecoupledIO(new AXI4BundleAR(edgeIn.bundle)))
      val r = DecoupledIO(new AXI4BundleR(edgeIn.bundle))
    }
    val out = new Bundle() {
  ...
  val readDataStack = Module(new SRAMTemplate(gen = UInt(axiDataBits.W), set = entries, way = 1, singlePort = true))
  ...
  */ val hasWaitTLReq = Cat(readStack.map(_.readStatus === waitSend)).orR
  axireqArb.io.in zip readStack foreach { case (in, e) =>
  ...
  */ val canReceive = Cat(readStack.map(e => e.rvalid && e.readStatus === waitResp)).orR
  ...
  private val d_source = io.out.d.bits.source
  private val resp_id = d_source(axi2tlParams.rbufIdBits - 1, 0)
  when(d_valid) {
    val entryResp = readStack(resp_id)
  ...
  readDataStack.io.w.apply(wen, io.out.d.bits.data, resp_id, 1.U)

  val dataWillWrite = RegNext(d_valid, false.B)
  val respIdx = RegEnable(resp_id, 0.U, d_valid)
  ...
  */ val priority = VecInit(readStack.map(e => e.readStatus === waitSendResp && e.rvalid && e.BeatFifoId === 0.U && e.rready))
  val proVec = priority.zip(readStack).map { case (valid, e) => Mux(valid, e.RespFifoId, entries.U)
  }
  val max_priority_fifoid = proVec.reduceLeft(_ min _)

  axirespArb.io.in zip readStack foreach { case (in, e) => in.valid := (e.readStatus === waitSendResp && e.rvalid && e.BeatFifoId === 0.U && e.RespFifoId === max_priority_fifoid && e.rready)
    in.bits := e
  }
  ...
  private val validCond = axirespArb.io.out.valid && !wen
  io.in.r.valid := RegNext(validCond, false.B)
  io.in.r.bits.data := RegEnable(readDataStack.io.r.resp.data(0), validCond)
  io.in.r.bits.id := RegEnable(axirespArb.io.out.bits.arid, validCond)
  io.in.r.bits.resp := RegEnable(axirespArb.io.out.bits.respStatus, validCond)
  ...
  when(io.in.ar.fire && io.out.a.fire) {
    val entry = readStack(idxInsert) // a entry status will be sending
