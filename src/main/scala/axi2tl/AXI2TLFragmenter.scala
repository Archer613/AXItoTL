...
/**
  ...
  */
class AXI2TLFragmenter()(implicit p: Parameters) extends LazyModule
{
  ...
  class Impl extends LazyModuleImp(this) {
    (node.in zip node.out) foreach { case ((in, edgeIn), (out, edgeOut)) =>
      ...
      def fragment(a: IrrevocableIO[AXI4BundleA], supportedSizes1: Seq[Int]): (IrrevocableIO[AXI4BundleA], Bool, UInt) = {
        ...
        //!!!
        val maxSupported1 = remain1 & align1 & support1 & 0.U // Take the minimum of all the limits
       ...
       def fragmentWrite(a: IrrevocableIO[AXI4BundleA], supportedSizes1: Seq[Int]): (IrrevocableIO[AXI4BundleA], Bool, UInt) = {
        val out = Wire(chiselTypeOf(a))
      ...
      val (in_aw, aw_last, w_beats) = fragmentWrite(Queue.irrevocable(in.aw, 1, flow=true), writeSizes1)
...
object AXI2TLFragmenter
{
  def apply()(implicit p: Parameters): AXI4Node =
  {
    val axi4frag = LazyModule(new AXI2TLFragmenter)
    axi4frag.node
  }
}
