...
case class MyAXI4ToTLNode(wcorrupt: Boolean)(implicit valName: ValName) extends MixedAdapterNode(AXI4Imp, TLImp)(
  dFn = {  mp => TLMasterPortParameters.v1(
      clients = mp.masters.map{m =>
          TLMasterParameters.v1(
            name        = s"axitotlnode",
            sourceId    = IdRange(0, 1<<(14+5)), // R+W ids are distinct
            nodePath    = m.nodePath,
            requestFifo = true
            )
      }
...
class  AXItoTL1(
  ...
  AXItoTLBundleOut : TLBundle
    ...
    val readStack = Module(new ReadStack(
    ...
    val writeStack = Module(new WriteStack(
      entries = axi2tlParams.readEntriesSize, 
      AXItoTLEdgeIn = AXItoTLEdgeIn,
      AXItoTLEdgeOut = AXItoTLEdgeOut,
      AXItoTLBundleIn= AXItoTLBundleIn,
